...
with lib; let
  cfg = config.lonewanderer.system.security.dnscrypt;
in {
  imports = [];

  options.lonewanderer.system.security.dnscrypt = {
  ...
  config = mkIf cfg.enable {
    ...
    services.resolved = mkIf config.hardware.network.enableNetworkManager {
      enable = true;
